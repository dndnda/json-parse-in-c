...
void lept_copy(lept_value* dst, const lept_value* src) {
    ...
    switch (src->type) {
        ...
        case LEPT_ARRAY:
            lept_free(dst);
            dst->u.a.e = (lept_value*)malloc(src->u.a.capacity * sizeof(lept_value));
        ...
        case LEPT_OBJECT:
            lept_free(dst);
            dst->u.o.m = (lept_member*)malloc(src->u.o.capacity * sizeof(lept_member));
            ...
            for (; i < src->u.o.size; i++) {
                ...
                memcpy(dst->u.o.m[i].k = malloc(src->u.o.m[i].klen * sizeof(char)), src->u.o.m[i].k, src->u.o.m[i].klen);
    ...
    switch (lhs->type) {
        ...
        case LEPT_OBJECT:
            ...
            for (i = 0; i < lhs->u.o.size; i++) {
                lept_value* rvalue = lept_find_object_value(rhs, lept_get_object_key(lhs, i), lept_get_object_key_length(lhs, i));
                if (rvalue == NULL || !lept_is_equal(&lhs->u.o.m[i].v, rvalue))
                    return 0;
            }
            return 1;
        default:
...
lept_value* lept_set_object_value(lept_value* v, const char* key, size_t klen) {
    lept_value* find_v;
    assert(v != NULL && v->type == LEPT_OBJECT && key != NULL);
    if ((find_v = lept_find_object_value(v, key, klen)) != NULL) {
        lept_free(find_v);
        return find_v;
    }
    else {
        ...
        memcpy(v->u.o.m[v->u.o.size].k = malloc(klen * sizeof(char)), key, klen);
